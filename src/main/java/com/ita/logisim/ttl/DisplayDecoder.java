...
public class DisplayDecoder extends InstanceFactory {
	...
	public static byte getdecval(InstanceState state, boolean multibit, int MultibitInputIndex, int Aindex, int Bindex,
		...
		if (!multibit && state.getPortValue(Aindex) != Value.UNKNOWN && state.getPortValue(Bindex) != Value.UNKNOWN
				&& state.getPortValue(Cindex) != Value.UNKNOWN && state.getPortValue(Dindex) != Value.UNKNOWN) {
			for (byte i = 0; i < 4; i++)
		...
		String text = (painter.getPortValue(7) == Value.FALSE) ? "!" + Strings.get("memEnableLabel")
				: painter
						.getPortValue(nports - 2) == Value.FALSE
								? "BI"
								: painter.getPortValue(nports - 3) == Value.FALSE ? "LI"
