...
public class VivadoDownload implements VendorDownload {
	...
	public VivadoDownload(String ProjectPath,
            	          FPGAReport Reporter,
            	          Netlist RootNetList,
	...
	public String GetStageMessage(int stage) {
		switch (stage) {
			case 0  : return S.get("VivadoProject");
			case 1  : return S.get("VivadoBitstream");
	...
	public boolean CreateDownloadScripts() {
		...
		File generateBitstreamFile = FileWriter.GetFilePointer(ScriptPath, GENERATE_BITSTREAM_FILE, Reporter);
		...
		contents.add("exit");
		if (!FileWriter.WriteContents(createProjectFile, contents, Reporter))
			return false;
		contents.clear();
		...
		if (!FileWriter.WriteContents(xdcFile, contents, Reporter))
			return false;
		contents.clear();
		...
		contents.add("exit");
		if (!FileWriter.WriteContents(generateBitstreamFile, contents, Reporter))
			return false;
		contents.clear();
