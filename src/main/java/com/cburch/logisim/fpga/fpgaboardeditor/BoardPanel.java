  ...
  public void paint(Graphics g) {
    ...
    if (image != null) {
      ...
      if ((w != 0) || (h != 0)) {
        ...
        hr = (h < 0) ? -h : h;
        g.drawRect(
            AppPreferences.getScaled(xr, scale),
            AppPreferences.getScaled(yr, scale),
            AppPreferences.getScaled(wr, scale),
            AppPreferences.getScaled(hr, scale));
      }
      ...
      if (EditMode) {
        ...
        message = image_width + "x" + image_height + " pixels (width x height)";
        xpos = (this.getWidth() - fm.stringWidth(message)) / 2;
