...
public class LoadedLibrary extends Library implements LibraryEventSource {
  ...
  LoadedLibrary(Library base) {
    ...
    while (base instanceof LoadedLibrary loadedLib) base = loadedLib.base;
    this.base = base;
    if (base instanceof LibraryEventSource eventSrc) {
      eventSrc.addLibraryListener(myListener);
    }
  }
  ...
  private void resolveChanges(Library old) {
    ...
    for (final var oldTool : old.getTools()) {
      ...
      if (oldTool instanceof AddTool at) {
        final var oldFactory = at.getFactory();
        if (newTool instanceof AddTool nat) {
          final var newFactory = nat.getFactory();
  ...
  void setBase(Library value) {
    if (base instanceof LibraryEventSource eventSrc) {
      eventSrc.removeLibraryListener(myListener);
    }
    final var old = base;
    base = value;
    resolveChanges(old);
