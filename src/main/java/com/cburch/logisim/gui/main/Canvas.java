...
public class Canvas extends JPanel implements LocaleListener, CanvasPaneContents {
  ...
  private class MyViewport extends JViewport {
    ...
    void paintContents(Graphics g) {
      ...
      if (canvasPane != null && panZoomHelper != null) {
        final var gMain = Canvas.this.getGraphics();
        final var bds = (gMain != null)
            ? proj.getCurrentCircuit().getBounds(gMain)
            : proj.getCurrentCircuit().getBounds();
        panZoomHelper.drawZoomAndArrows(g, sz, bds, getViewableBaseRect(), getZoomFactor());
      }
