...
/**
 * Handles graphical actions and events on Chronoframe Left area: SignalNames RightArea: TimeLine
 * and SignalDraw
 */
public class DrawAreaManager implements IDrawAreaEvents {
  ...
  /**
   * Draw the red vertical line on every signal If no PoxX is given, redraw the line at the same
   * position
   */
   ...
   *
   * @param posX position of the red vertical line
   */
  ...
  public void highlight(SignalData signalToHighlight) {
    if (mChronoFrame.getRightPanel() != null && mChronoFrame.getLeftPanel() != null) {
   ...
   * Mouse dragged
   *
   * @param signalDataSource SignalData that correspond to the object that trigger the event
   * @param posX position of cursor on X axis
   */
  @Override
  ...
  public void refreshSignalsValues() {
    if (mChronoFrame.getLeftPanel() != null && mChronoFrame.getRightPanel() != null) {
   ...
   *
   * @param signalDataSource SignalDataBus that correspond to the bus that trigger the event and/or
   ...
   * Refresh the display of each signal value in the left bar
   *
   * @param posX the position of the cursor. it defines the value to display
   */
   ...
   * @param signalDataSource SignalDataBus that correspond to the bus that trigger the event and/or
   *     the bus to apply the transformation
   * @param expand if true expand the bus into multiple signals, close it otherwise
   */
  @Override
  ...
  /** Zoom in or zoom out the signal Focus on the red vertical line (mousePosXClicked) */
   ...
   *
   * @param signalDataSource SignalData that correspond to the object that trigger the event
   * @param sens 1=zoom in, -1=zoom out
   * @param posX position of cursor on X axis
   */
  @Override
