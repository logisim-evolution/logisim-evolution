...
public class TruthtableCsvFile {
  ...
  public static final FileFilter FILE_FILTER = new TruthtableFileFilter(S.getter("tableCsvFileFilter"), ".csv");
  ...
  public static void doSave(File file, AnalyzerModel model) throws IOException {
    ...
    try (val out = new PrintStream(file)) {
      val tt = model.getTruthTable();
      tt.compactVisibleRows();
      for (var i = 0; i < inputs.vars.size(); i++) {
        val cur = inputs.vars.get(i);
        val name = cur.width == 1 ? cur.name : cur.name + "[" + (cur.width - 1) + "..0]";
  ...
  public static void doLoad(File file, AnalyzerModel model, JFrame parentFrame) throws IOException {
    val param = new CsvParameter();
    // FIXME What is this initialization used for? Smell a bit.
    new CsvReadParameterDialog(param, file, parentFrame);
    if (!param.isValid()) return;
    val cin = new CsvInterpretor(file, param, parentFrame);
