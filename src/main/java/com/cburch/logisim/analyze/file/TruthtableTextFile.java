...
public class TruthtableTextFile {
  ...
  public static void doSave(File file, AnalyzerModel model) throws IOException {
    try (PrintStream out = new PrintStream(file)) {
      ...
      var i = 0;
      for (var variable : inputs.vars) colwidth[i++] = Math.max(variable.toString().length(), variable.width);
      for (var variable : outputs.vars) colwidth[i++] = Math.max(variable.toString().length(), variable.width);
      i = 0;
