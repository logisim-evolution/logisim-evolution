...
public class KMapGroups {
	...
	public static class CoverColor {
		...
		private final String[] CoverColorNames = {"MyMaroon",
				                                  "MyRed",
				                                  "MyPink",
				                                  "MyBrown",
				                                  "MyOrange",
		...
		public String getColorName(Color col) {
			if (colors.contains(col))
				return colorNames.get(colors.indexOf(col));
			return null;
		}
	...
	public class KMapGroupInfo {
		...
		public void removeSingleCover(Implicant imp) {
			if (singleCoveredImplicants.contains(imp))
		...
		public void paint(Graphics2D g , int x , int y , int cellWidth , int cellHeight, boolean highlighted, boolean colored) {
			...
			for (CoverInfo cover : Areas) {
				g.fillRoundRect(x+cover.getCol()*cellWidth+IMP_INSET, 
						y+cover.getRow()*cellHeight+IMP_INSET, 
						cover.getWidth()*cellWidth-2*IMP_INSET, 
						cover.getHeight()*cellHeight-2*IMP_INSET, d, d);
			}
			g.setColor(col);
		}
	...
	public boolean highlight(int col , int row) {
		...
		for (int nr = 0 ; nr < covers.size() && highlighted < 0 ; nr++) {
			if (covers.get(nr).insideCover(col, row)) 
				highlighted = nr;
		}
