...
public class Propagator {
  ...
  private volatile Thread propatatorThread;
  ...
  public Propagator(CircuitState root) {
    ...
    propatatorThread = root.getProject().getSimulator().simThread;
    updateRandomness();
    updateSimLimit();
  }
  ...
  /** Should only be called if the simulator will not run. Should be called with null when finished. */
  public void setPropagatorThread(Thread propagatorThread) {
    this.propatatorThread = propagatorThread != null ? propagatorThread : root.getProject().getSimulator().simThread;
  }
  ...
  public boolean propagate(Simulator.ProgressListener propListener, Simulator.Event propEvent) {
    if (Thread.currentThread() != propatatorThread) {
