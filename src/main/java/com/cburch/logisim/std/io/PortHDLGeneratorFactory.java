...
public class PortHDLGeneratorFactory extends AbstractHDLGeneratorFactory {
  ...
  private HashMap<String, HashMap<Integer, InOutMap>> compMap =
      new HashMap<String, HashMap<Integer, InOutMap>>();
  ...
  public ArrayList<String> GetEntity(
    ...
    for (int portNr = 0; portNr < ComponentInfo.GetComponent().getEnds().size(); portNr++) {
      Location splitterLoc =
          findEndConnection(
              ComponentInfo.GetComponent().getEnd(portNr).getLocation(), TheNetlist.getCircuit());
      if (splitterLoc == null) {
        Reporter.AddFatalError(
            "Found 0, 2 or more connections on PortIO's splitter (" + ComponentName + ")");
        return null;
      }
      for (Splitter split : TheNetlist.getSplitters()) {
        if (split.getLocation().equals(splitterLoc)) { // trouve le
          ...
          compMap
              ...
              .put(
                  mapIdx,
                  new InOutMap(Type.INOUT, new Point(0, split.GetEndpoints().length - 1), portNr));
          int splitPortNr = 0;
          for (EndData end : split.getEnds()) {
            if (!end.getLocation().equals(splitterLoc)) { // parcours
              ...
              for (Component comp : TheNetlist.getCircuit().getNonWires(compLoc)) { // parcours le
                // (les?)
                // composant
                // connecte a la
                // sortie du
                ...
                for (EndData port : comp.getEnds()) {
                  if (port.getLocation().equals(compLoc)) { // trouve
                    ...
                    if (!(comp instanceof Splitter) && !(comp instanceof PortIO)) {
                      if (port.isInput()) {
                        compMap
                            ...
                            .put(
                                ...
                                new InOutMap(
                                    Type.OUT,
                                    getBitRange(split.GetEndpoints(), splitPortNr),
                                    portNr));
                      ...
                      Reporter.AddFatalError(
                          "Cannot connect PortIO's splitter to other splitter or PortIO ("
                              + ComponentName
                              + ")");
                      return null;
                    }
    ...
    for (int i = 0; i < compMap.get(ComponentName).size(); i++) {
      ...
      switch (compMap.get(ComponentName).get(i).getType()) {
        ...
        default:
          Reporter.AddFatalError(
              "Found component of unknown type ("
      ...
      if (compMap.get(ComponentName).get(i).getSize() == 1) {
        ...
        line +=
            "std_logic_vector (" + (compMap.get(ComponentName).get(i).getSize() - 1) + " DOWNTO 0)";
      }
  ...
  public SortedMap<String, Integer> GetInOutList(Netlist TheNetlist, AttributeSet attrs) {
    ...
    for (int i = 0; i < compMap.get(ComponentName).size(); i++) {
      if (compMap.get(ComponentName).get(i).getType() == Type.INOUT) {
        InOuts.put(inOutBusName + "_" + i, compMap.get(ComponentName).get(i).getSize());
      }
    }
  ...
  public SortedMap<String, Integer> GetInputList(Netlist TheNetlist, AttributeSet attrs) {
    ...
    for (int i = 0; i < compMap.get(ComponentName).size(); i++) {
      if (compMap.get(ComponentName).get(i).getType() == Type.IN) {
        Inputs.put(inBusName + "_" + i, compMap.get(ComponentName).get(i).getSize());
      }
    }
  ...
  public SortedMap<String, Integer> GetOutputList(Netlist TheNetlist, AttributeSet attrs) {
    ...
    for (int i = 0; i < compMap.get(ComponentName).size(); i++) {
      if (compMap.get(ComponentName).get(i).getType() == Type.OUT) {
        Outputs.put(outBusName + "_" + i, compMap.get(ComponentName).get(i).getSize());
      }
    }
        ...
        case IN:
          ...
          name =
              BusName
                  + Integer.toString(
                      Nets.GetNetId(
                          ComponentInfo.getEnd(compMap.get(ComponentName).get(i).getPortNb())
