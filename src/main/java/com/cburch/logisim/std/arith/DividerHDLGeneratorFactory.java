...
public class DividerHDLGeneratorFactory extends AbstractHDLGeneratorFactory {
  ...
  final private static String UnsignedStr = "UnsignedDivider";
  ...
  public ArrayList<String> GetModuleFunctionality(Netlist TheNetlist,
    ...
    if (HDLType.equals(VHDL)) {
        ...
        Contents.add("                   std_logic_vector(signed(s_extended_dividend) / signed(INP_B));");
        Contents.add("   s_mod_result <= std_logic_vector(unsigned(s_extended_dividend) mod unsigned(INP_B))");
        Contents.add("                      WHEN "+UnsignedStr+" = 1 ELSE");
        Contents.add("                   std_logic_vector(signed(s_extended_dividend) mod signed(INP_B));");
