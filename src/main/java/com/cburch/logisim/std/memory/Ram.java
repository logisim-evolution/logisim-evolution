...
public class Ram extends Mem {
  ...
  public static int ByteEnableIndex(AttributeSet Attrs) {
    ...
    if (byteEnables) {
      int ByteEnableIndex =
          (asynch) ? (separate) ? AByEnSep : AByEnBiDir : (separate) ? SByEnSep : SByEnBiDir;
  ...
  private void DrawConnections(
    ...
    if (separate) {
      if (singleBit) {
        ...
        g.drawLine(xpos + 20 + SymbolWidth, ypos + 10, xpos + 40 + SymbolWidth, ypos + 10);
      } else {
      ...
      String DLabel = (sync) ? "A,1,3" + ByteIndex + "D" : "A,1" + ByteIndex + "D";
      String QLabel = (sync) ? "A,2,3" + ByteIndex : "A,2" + ByteIndex;
      g.setFont(font.deriveFont(9.0f));
      GraphicsUtil.drawText(
          g, DLabel, xpos + 23, ypos + 10, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
      GraphicsUtil.drawText(
          g,
          QLabel,
          xpos + 17 + SymbolWidth,
  ...
  private void DrawControlBlock(InstancePainter painter, int xpos, int ypos) {
    ...
    GraphicsUtil.drawText(
        g, "M1 [Write Enable]", xpos + 33, ypos + 50, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
    painter.drawPort(WE);
    if (asynch && inverted) {
    ...
    GraphicsUtil.drawText(
        g, "M2 [Output Enable]", xpos + 33, ypos + 60, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
    painter.drawPort(OE);
    if (!asynch) {
      int yoffset = 70;
      if (byteEnables) {
      ...
      GraphicsUtil.drawText(
          g, "C3", xpos + 33, ypos + yoffset, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
      ...
      for (int i = 0; i < NrOfByteEnables; i++) {
        ...
        String Label =
            "M"
                ...
                + "]";
        GraphicsUtil.drawText(
            g, Label, xpos + 33, ypos + 70 + i * 10, GraphicsUtil.H_LEFT, GraphicsUtil.V_CENTER);
      }
    }
    GraphicsUtil.switchToWidth(g, 1);
  ...
  private void DrawDataBlock(InstancePainter painter, int xpos, int ypos, int bit, int NrOfBits) {
    ...
    DrawConnections(g, xpos, realypos, FirstBlock & LastBlock, separate, !asynch, byteEnables, bit);
    if (FirstBlock) {
  ...
  public Bounds getOffsetBounds(AttributeSet attrs) {
    ...
    if (attrs.getValue(StdAttr.APPEARANCE) == StdAttr.APPEAR_CLASSIC) {
      ...
      return Bounds.create(0, 0, SymbolWidth + xoffset, getControlHeight(attrs) + 20 * len);
    }
  }
  ...
  protected void instanceAttributeChanged(Instance instance, Attribute<?> attr) {
    ...
    if ((attr == Mem.DATA_ATTR)
      ...
      if ((attr == Mem.DATA_ATTR) || (attr == StdAttr.TRIGGER)) {
        boolean disable_due_to_bits = instance.getAttributeValue(Mem.DATA_ATTR).getWidth() < 9;
        boolean disable_due_to_async =
            instance.getAttributeValue(StdAttr.TRIGGER).equals(StdAttr.TRIG_HIGH)
                || instance.getAttributeValue(StdAttr.TRIGGER).equals(StdAttr.TRIG_LOW);
        if (disable_due_to_bits || disable_due_to_async) {
          if (!instance
              .getAttributeValue(RamAttributes.ATTR_ByteEnables)
              .equals(RamAttributes.BUS_WITHOUT_BYTEENABLES)) {
            instance
                .getAttributeSet()
                .setValue(RamAttributes.ATTR_ByteEnables, RamAttributes.BUS_WITHOUT_BYTEENABLES);
          ...
          if (instance.getAttributeSet().isReadOnly(RamAttributes.ATTR_ByteEnables)) {
  ...
  public void DrawRamClassic(InstancePainter painter) {
    ...
    if (separate) {
      if (asynch) {
        painter.drawPort(ADIN, S.get("ramDataLabel"), Direction.EAST);
      } else {
        painter.drawPort(SDIN, S.get("ramDataLabel"), Direction.EAST);
      }
    }
    Object be = painter.getAttributeValue(RamAttributes.ATTR_ByteEnables);
    boolean byteEnables = be == null ? false : be.equals(RamAttributes.BUS_WITH_BYTEENABLES);
    if (byteEnables) {
      ...
      for (int i = 0; i < NrOfByteEnables; i++) {
        painter.drawPort(
            ByteEnableIndex + i, S.get("ramWELabel") + (NrOfByteEnables - i - 1), Direction.EAST);
      }
    }
  }
  ...
  public void propagate(InstanceState state) {
    ...
    boolean outputEnabled =
        (!asynch || trigger.equals(StdAttr.TRIG_HIGH))
