...
public class Ttl74299 extends AbstractTtlGate {
  ...
  private record LogicScope(InstanceState state, ShiftRegisterData data) {
    ...
    private void propagateRegister() {
      if (getPort(nCLR) == Value.FALSE) { // CLR is active low and clear is async
        ...
        switch (getMode()) {
          case LOAD:
            for (var i = 0; i < 8; i++) {
              data.set(i, getPort(DATA[i]));
            }
            break;
          case SHIFT_LEFT:
            data.pushUp(getPort(SL));
            break;
          case SHIFT_RIGHT:
            data.pushDown(getPort(SR));
            break;
          case HOLD:
            // Nothing to do
            break;
    ...
    private void propagateOutputs() {
      setPort(QA, data.get(7));  // Most significant bit
      setPort(QH, data.get(0));  // Least significant bit

      for (var i = 0; i < 8; i++) {
        setPort(DATA[i], isOutputEnabled() ? data.get(i) : Value.UNKNOWN);
      }
    }
