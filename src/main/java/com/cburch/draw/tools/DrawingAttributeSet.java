...
public class DrawingAttributeSet implements AttributeSet, Cloneable {
  ...
  public Object clone() {
    try {
      val ret = (DrawingAttributeSet) super.clone();
  ...
  public <V> void setValue(Attribute<V> attr, V value) {
    ...
    while (ait.hasNext()) {
      ...
      if (a.equals(attr)) {
        vit.set(value);
        var evt = new AttributeEvent(this, attr, value, null);
        for (val listener : listeners) {
          listener.attributeValueChanged(evt);
        }
        if (attr == DrawAttr.PAINT_TYPE) {
          evt = new AttributeEvent(this);
          for (val listener : listeners) {
            listener.attributeListChanged(evt);
          }
        }
        return;
      }
