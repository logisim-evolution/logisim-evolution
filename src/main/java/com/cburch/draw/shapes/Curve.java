...
public class Curve extends FillableCanvasObject {
  ...
  public boolean contains(Location loc, boolean assumeFilled) {
    ...
    if (type != DrawAttr.PAINT_FILL) {
      ...
      val q = toArray(loc);
      val p0 = toArray(this.p0);
      val p1 = toArray(this.p1);
      val p2 = toArray(this.p2);
      val p = CurveUtil.findNearestPoint(q, p0, p1, p2);
      if (p == null) return false;

      val thr =
          (type == DrawAttr.PAINT_STROKE) ? Math.max(Line.ON_LINE_THRESH, stroke / 2) : stroke / 2;
    ...
    if (gesture == null) {
      ...
      if (g.isAt(p0)) {
        if (gesture.isShiftDown()) {
          val p = LineUtil.snapTo8Cardinals(p2, gx, gy);
        ...
        if (gesture.isShiftDown()) {
          val p = LineUtil.snapTo8Cardinals(p0, gx, gy);
        ...
        if (gesture.isShiftDown()) {
          ...
          val dx = x1 - x0;
          val dy = y1 - y0;
          val p = LineUtil.nearestPointInfinite(gx, gy, midx, midy, midx - dy, midy + dx);
