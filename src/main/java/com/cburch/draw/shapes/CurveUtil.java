...
public class CurveUtil {
  ...
  public static double[] findNearestPoint(double[] q, double[] p0, double[] p1, double[] p2) {
    val A = computeA(p0, p1);
    val B = computeB(p0, p1, p2);

    // a temporary util vect = p0 - (x,y)
    double[] pos = {p0[0] - q[0], p0[1] - q[1]};
    // search points P of bezier curve with PM.(dP / dt) = 0
    // a calculus leads to a 3d degree equation :
    val a = B[0] * B[0] + B[1] * B[1];
    val b = 3 * (A[0] * B[0] + A[1] * B[1]);
    val c = 2 * (A[0] * A[0] + A[1] * A[1]) + pos[0] * B[0] + pos[1] * B[1];
    val d = pos[0] * A[0] + pos[1] * A[1];
  ...
  public static Bounds getBounds(double[] p0, double[] p1, double[] p2) {
    ...
    val x = (int) xMin;
    val y = (int) yMin;
    val w = (int) Math.ceil(xMax) - x;
    val h = (int) Math.ceil(yMax) - y;
  ...
  public static double[] interpolate(double[] end0, double[] end1, double[] mid) {
    ...
    val xNum = mid[0] - u2 * end0[0] - t2 * end1[0];
    val yNum = mid[1] - u2 * end0[1] - t2 * end1[1];
  ...
  private static double[] solveCubic(double a, double b, double c, double d) {
    if (Math.abs(a) > zeroMax) {
      ...
      if (D > zeroMax) {
        ...
        // D negative
        val u = 2 * Math.sqrt(-p / 3);
        val v = Math.acos(-Math.sqrt(-27 / p3) * q / 2) / 3;
        return new double[] {
          u * Math.cos(v) + offset,
        ...
        // D zero
        val u = (q < 0) ? Math.pow(-q / 2, 1. / 3) : -Math.pow(q / 2, 1. / 3);
