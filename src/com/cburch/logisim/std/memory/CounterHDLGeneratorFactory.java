...
public class CounterHDLGeneratorFactory extends AbstractHDLGeneratorFactory {
	...
	public ArrayList<String> GetModuleFunctionality(Netlist TheNetlist,
		...
		if (HDLType.equals(Settings.VHDL)) {
			...
			Contents.add("         IF (s_counter_value = conv_std_logic_vector(0,width)) THEN");
			Contents.add("            s_carry <= '1';");
			Contents.add("                                                               ELSE");
			Contents.add("            s_carry <= '0';");
			Contents.add("         END IF; -- Down counting");
			Contents.add("                           ELSE");
			Contents.add("         IF (s_counter_value = conv_std_logic_vector(max_val,width)) THEN");
			Contents.add("            s_carry <= '1';");
			Contents.add("                                                                     ELSE");
			Contents.add("            s_carry <= '0';");
			...
			Contents.add("                                 s_next_counter_value <= conv_std_logic_vector(max_val,width);");
			Contents.add("                                                     ELSE ");
			Contents.add("                                 s_next_counter_value <= (OTHERS => '0');");
			Contents.add("                              END IF;");
			Contents.add("                                              ELSE");
			Contents.add("                              IF (v_downcount = '1') THEN ");
			Contents.add("                                 s_next_counter_value <= unsigned(s_counter_value) - 1;");
			Contents.add("                                                     ELSE ");
			Contents.add("                                 s_next_counter_value <= unsigned(s_counter_value) + 1;");
			Contents.add("                              END IF;");
			Contents.add("                           END IF;");
			...
			Contents.add("      ELSIF (GlobalClock'event AND (GlobalClock = conv_std_logic_vector(ClkEdge,1)(0))) THEN");
			...
			Contents.add("   assign CountValue = (ClkEdge) ? s_counter_value : s_counter_value_neg_edge;");
			Contents.add("");
			Contents.add("   always@(*)");
			Contents.add("   begin");
			Contents.add("      if (Up_n_Down)");
			Contents.add("         begin");
			Contents.add("            if (ClkEdge)");
