...
public class Ttl7410 extends AbstractTtlGate {
	...
	public void ttlpropagate(InstanceState state) {
		Value val = state.getPortValue(2).and(state.getPortValue(3).and(state.getPortValue(4)));
		state.setPort(5, inverted ? val.not() : val , 2);
		val = state.getPortValue(0).and(state.getPortValue(1).and(state.getPortValue(11)));
		state.setPort(10, inverted ? val.not() : val, 2);
		val = state.getPortValue(7).and(state.getPortValue(8).and(state.getPortValue(9)));
	...
	public boolean HDLSupportedComponent(String HDLIdentifier,
		...
		if (MyHDLGenerator == null)
			MyHDLGenerator = new Ttl7410HDLGenerator();
